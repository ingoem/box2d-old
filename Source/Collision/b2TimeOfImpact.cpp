					   ...
					   float32 maxTOI)
{
	...
	for (int32 iter = 0; iter < maxIterations; ++iter)
	{
...
#if 0
		float32 vn = b2Dot(normal, v2 + b2Cross(omega2, p2 - x2) - v1 - b2Cross(omega1, p1 - x1));
		if (distance < b2_linearSlop && vn < FLT_EPSILON)
		{
			break;
		}
#else
		...
		float32 dt;
#if 0
		if (distance > 0.5f * b2_linearSlop + FLT_EPSILON)
		{
			dt = (distance - 0.5f * b2_linearSlop) / relativeVelocity;
		}
		else
		{
			dt = distance / relativeVelocity;
		}
#else
		dt = (distance - 0.5f * b2_linearSlop) / relativeVelocity;
#endif
