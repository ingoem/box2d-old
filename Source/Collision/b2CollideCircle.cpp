...
void b2CollideCircles(b2Manifold* manifold, b2CirclePrimitive* circle1, b2CirclePrimitive* circle2)
{
	manifold->pointCount = 0;
...
void b2CollidePolygonAndCircle(b2Manifold* manifold, const b2PolygonPrimitive* polygon, const b2CirclePrimitive* circle)
{
	...
	// Compute circle position in the frame of the polygon.
	b2Vec2 xLocal = b2MulT(polygon->m_R, circle->m_position - polygon->m_position);
	...
	{
		float32 s = b2Dot(polygon->m_normals[i], xLocal - polygon->m_vertices[i]);
		if (s > radius)
		{
			// Early out.
			return;
	...
	{
		manifold->pointCount = 1;
		manifold->normal = b2Mul(polygon->m_R, polygon->m_normals[normalIndex]);
		manifold->points[0].id.features.incidentEdge = (uint8)normalIndex;
		manifold->points[0].id.features.incidentVertex = b2_nullFeature;
		manifold->points[0].id.features.referenceFace = b2_nullFeature;
		manifold->points[0].id.features.flip = 0;
	...
	if (length < FLT_EPSILON)
	{
		b2Vec2 d = xLocal - polygon->m_vertices[vertIndex1];
		float32 dist = d.Normalize();
		if (dist > radius)
		{
			return;
		...
		manifold->pointCount = 1;
		manifold->normal = b2Mul(polygon->m_R, d);
		manifold->points[0].id.features.incidentEdge = b2_nullFeature;
		manifold->points[0].id.features.incidentVertex = (uint8)vertIndex1;
		manifold->points[0].id.features.referenceFace = b2_nullFeature;
		manifold->points[0].id.features.flip = 0;
	...
	// Project the center onto the edge.
	float32 u = b2Dot(xLocal - polygon->m_vertices[vertIndex1], e);
	manifold->points[0].id.features.incidentEdge = b2_nullFeature;
	manifold->points[0].id.features.incidentVertex = b2_nullFeature;
	manifold->points[0].id.features.referenceFace = b2_nullFeature;
	manifold->points[0].id.features.flip = 0;
